...
<critical>PASSO 0, ANTES DE QUALQUER CÓDIGO: abra `creed-backend/app/domains/<dominio>/router.py`. Docstring "STUB" ou diretório inexistente = o contrato NÃO existe. Hoje NENHUM domínio tem contrato acordado: cinco são stubs registrados no main.py que respondem 404, e `respondentes` é exemplo gerado no scaffold — copie dele a forma, nunca os campos. Nesse caso PARE, diga qual arquivo você abriu, e trate o contrato como decisão de produto: spec + premissa no ledger + tarefa de backend no ClickUp. Nunca invente campo em silêncio.</critical>
